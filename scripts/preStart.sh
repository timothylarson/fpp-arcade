...
# Nothing to do here in the normal case.  fppd loads libfpp-arcade.so once at its own
...
if [ ! -f libfpp-arcade.so ]; then
    echo "Running fpp-arcade PreStart Script - libfpp-arcade.so is missing, rebuilding"
    make
fi
